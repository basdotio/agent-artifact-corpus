...
```bash
codex exec --yolo -m gpt-5.4 -c model_reasoning_effort="xhigh" --skip-git-repo-check "$(cat <<'ENDCODE'
...
|------|--------|-------------|
| `-m, --model` | `gpt-5.4` | Specify model (gpt-5.4 is the current version) |
...
1. **Stdin piping** (recommended):
   ```bash
   cat target.py | codex exec -m gpt-5.4 -c model_reasoning_effort="medium" \
     --skip-git-repo-check --full-auto - 2>/dev/null
   ```
...
3. Check current supported model: see [Codex CLI documentation](https://developers.openai.com/codex/cli/reference)
...
3. **Heredoc for Long Code**:
   ```bash
   codex exec --yolo -m gpt-5.4 -c model_reasoning_effort="high" --skip-git-repo-check "$(cat <<'EOF'
...
4. **Run on Login Node** (if compute node blocks outbound):
   ...
   codex exec --yolo -m gpt-5.4 --skip-git-repo-check "analyze @file.py" 2>/dev/null
   ```
...
|-----------|------------------|------------------|
| Quick syntax fixes | `gpt-5.4` | none |
| Simple refactoring | `gpt-5.4` | low |
| Code review | `gpt-5.4` | high |
| Complex refactoring | `gpt-5.4` | high |
| Architecture analysis | `gpt-5.4` | xhigh |
| Security audit | `gpt-5.4` | high |
| Algorithm optimization | `gpt-5.4` | xhigh |
| Documentation generation | `gpt-5.4` | low |
| Tough algorithmic problems | `gpt-5.4` | xhigh |
| High-throughput tasks | `gpt-5.4` | none |
