...
that. Of 3,509 samples, 253 now carry a quote located in the artifact's own bytes — which is
...
| Duplication & integrity | exact and near duplicates, cross-class contamination, byte integrity against pinned upstreams | all 3,509 trees; 6,084,816 pairs exactly |
...
- **`leakage active over 3509 samples`** reported the corpus's size as if it were the gate's
